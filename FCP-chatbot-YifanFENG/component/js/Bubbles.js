...
function Bubbles(container, self, options) {
  ...
  let addBubble = function(say, posted, human_reply, live) {
	  // console.log(say,posted, human_reply, live)
    ...
    if (human_reply !== "") {
      ...
      for (let z = 0; z < bubbleButtons.length; z++) {
		  let left = bubbleButtons[z].offsetLeft;
		  let top = bubbleButtons[z].offsetTop;
		  let height = bubbleButtons[z].offsetHeight;
		  console.log('top==>',top)
		  // create horn
		  let horn = document.createElement("img");
		  horn.src="img/horn.png";
		  horn.className = 'horn hornReplay';
		  horn.style.left = (left-30)+'px';
		  horn.style.top = top+height-5+'px';
		  horn.onclick = function(){
			  window.event? window.event.cancelBubble = true : e.stopPropagation();
			  console.log('bubbleButtons==>',bubbleButtons[z].innerHTML);
			  let u = new SpeechSynthesisUtterance();
			  u.lang = "en-US"
			  u.volume = 50;
			  u.rate = 1;
			  u.pitch = 0.8;
			  u.text = bubbleButtons[z].innerHTML;
			  speechSynthesis.speak(u)
		  }
		bubbleContent.appendChild(horn)
        ;(function(el) {
      ...
      bubble.addEventListener("click", function() {
		  let horns = bubble.querySelectorAll('.horn')
		  console.log(horns)
   ...
   //          el.style.width = 0 + "px";
			...
			horns[i].style.display="none";
			if(el.classList.contains("bubble-pick")){
				// el.style.width = "";
				// el.style.height = "";
				el.style.display="inline-block";
				horns[i].style.display="block";
				console.log(el,el.offsetLeft,el.offsetTop);
				horns[i].style.left = (el.offsetLeft-30)+'px';
				horns[i].style.top = el.offsetTop+el.offsetHeight-10+'px';
			}
